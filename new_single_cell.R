...
# new code
# importing metadata with clusters; changing cell barcodes to match
...
# changing cell barcodes to match
crohns_meta_all[,1] <- sub("-.*", "", crohns_meta_all[,1])
crohns_meta_all[,1] <- sub(" ", "", crohns_meta_all[,1])

# merging the dataframes by cell; only selecting the columns we need
...
# adding empty clusters column to HIV_meta_all and changing cell barcodes to match
...
# combining HIV and crohns into exprs_norm by common row names
...
p6 <- ggplot(umap_labels1[sample(nrow(umap_labels1)),],
...
# make labelled clusters graph
  cluster_center <- umap_labels1 %>%
  group_by(clusters) %>%
  summarise_at(vars(UMAP1, UMAP2), funs(median(., na.rm=TRUE)))
cluster_center <- as.data.frame(cluster_center)
cluster_center$clusters <- as.character(cluster_center$clusters)

p8 <- ggplot(umap_labels1[sample(nrow(umap_labels)),],
             aes(x = UMAP1, y = UMAP2, fill= clusters)
) +
...
# plot all graphs in a grid
options(repr.plot.height = 5, repr.plot.width = 15)
plot_grid(p1+ theme(text = element_text(size = 10)), p2+ theme(text = element_text(size = 10)), 
          p4+ theme(text = element_text(size = 10)), p5+ theme(text = element_text(size = 10)), 
          p7+ theme(text = element_text(size = 10)), p8+ theme(text = element_text(size = 10)), 
